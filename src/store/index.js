      ...
      db.ref('/clients').on('value', snapshot => {
        ...
        if (snapshot.val()) {
          for (const key in snapshot.val()) {
            ...
            clients.push({ name: client.name, characteristics: client.characteristics, products: client.products })
      ...
      db.ref('/products').on('value', snapshot => {
        ...
        if (snapshot.val()) {
          for (const key in snapshot.val()) {
            ...
            products.push({ name: product.name, characteristics: product.characteristics, sourceMaterials: product.sourceMaterials })
